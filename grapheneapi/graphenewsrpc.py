...
class GrapheneWebsocketRPC(object):
    ...
    def wsconnect(self):
        ...
        while True:
            cnt += 1
            # heshipeng add 3 time after app continue run
            if cnt > 3:
                break
    ...
    def rpcexec(self, payload):
        ...
        while True:
            ...
            # heshipeng add 3 times after continue
            if cnt > 3:
                raise Exception("after try 3 times,Sure send init fail")
                break
        ...
        if 'error' in ret:
            if 'detail' in ret['error']:
                # heshipeng mod
                print(RPCError(ret['error']['detail']))
                raise RPCError(ret['error']['detail']) 
            else:
                # heshipeng mod
                print(RPCError(ret['error']['message']))
